...
export default function App() {
  ...
  const addcartitems = async (item) => {
    const response = await fetch('http://192.168.0.104:5000/cart/add', {
      ...
      headers: {
        ...
        'auth-token': "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiNjNiZmU0YzUzZDRlMmU0MWUxN2YyNDg1In0sImlhdCI6MTY3NTQ5NTIzNH0.xwKxleSNdXOO7QsoZFF-uALkBkeNnHjWJGwEqM3Sn6U"
      },
  ...
  const getcartitems = async () => {
    const response = await fetch('http://192.168.0.104:5000/cart/get', {
      ...
      headers: {
        'Content-Type': 'application/json',
        "auth-token":    const token = await AsyncStorage.getItem('token')
