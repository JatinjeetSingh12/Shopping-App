...
export default function App() {
  ...
  const addcartitems = async (item) => {
    const response = await fetch('http://:5000/cart/add', {
      method: 'POST',
      headers: {
  ...
  const getcartitems = async () => {
    const response = await fetch('http://192.168.0.102:5000/cart/get', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        "auth-token": await AsyncStorage.getItem('token')
  ...
  const deletecartitems = async (id) => {
    const response = await fetch(`http://192.168.0.102:5000/cart/delete/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': await AsyncStorage.getItem('token')
  ...
  return (
    <ShopContext.Provider value={{ login, logout, addcartitems, getcartitems, Cart, deletecartitems }}>
      <NavigationContainer>
